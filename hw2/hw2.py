...
def read_mongo_config(filename='./config/mongo.json'):
    if(path.exists(filename)): # path valid
        with open(filename) as f:
            try:
                data = json.load(f)
...
class MyHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def _audit(self, re, obj):
        headers = dict(self.headers)
        reqContentLength = self.headers['Content-Length'] if 'Content-Length' in self.headers else 0
        # print(reqContentLength)
        # # re = dict(re)
        # # print(re)
        # print("--audit--")
        ...
        log = {
            "request_id": str(obj['req_id']),
            "is_sensetive": True,
            "request_time": str(obj['start_time'].replace(microsecond=0).astimezone().isoformat()), 
            "request_duration_ms": obj['duration'].total_seconds() * 1000,
            "request_ip": str(self.client_address[0]),
            "requset_user_agent": str(headers['User-Agent']),
            "request_method": str(self.command),
            "request_headers": headers,
            "request_body": self.rfile.read(int(reqContentLength)).decode('utf-8'),
            "request_path": str(self.path),
            "request_query_string": str(self.qs),
            "host": str(headers['Host']),
            "response_headers": res_header,
            "response_http_code": int(re[0]),
            "response_body": obj['res_body'],
        ...
        if int(re[0]) >= 400:
            log['response_is_error'] = True


        # log['']
        #log = json.dumps(log)
        #print(mongo_config)
        mongoDB[mongo_config["coll"]].insert_one(log)
        print(log)
    ...
    def do_GET(self):
        ...
        req_start_time.replace(microsecond=0).astimezone().isoformat()

        ##
        r = self._do_request()
        req_end_time = datetime.datetime.now()
        req_end_time.replace(microsecond=0).astimezone().isoformat()
        #r = dict(r)

        res_body = {}
        req = {}

        req['request_time'] = str(req_start_time.replace(microsecond=0).astimezone().isoformat())
...
client = MongoClient(mongo_config["host"], int(mongo_config["port"]))
mongoDB = client[mongo_config["db"]]
